...
def create_user_input():
    ...
    median_income = st.sidebar.number_input('Median Income', min_value=499.9, max_value=150001, value=50000, step=1)
    rooms_per_household = st.sidebar.number_input('Rooms per Household', min_value=2.059769, max_value=8.440329, value=3, step=0.01)
    bedrooms_per_room = st.sidebar.number_input('Bedrooms per Room', min_value=0.079867, max_value=0.335025, value=0.1, step=0.01)
    population_per_household = st.sidebar.number_input('Population per Household', min_value=1.147406, max_value=4.560277, value=2, step=0.01)
    income_per_household = st.sidebar.number_input('Income per Household', min_value=0.379596, max_value=27.629386, value=5, step=0.01)
    price_per_room = st.sidebar.number_input('Price per Room', min_value=2.255639, max_value=274.845958, value=50, step=0.01)
    rooms_per_person = st.sidebar.number_input('Rooms per Person', min_value=0.372505, max_value=3.449184, value=1, step=0.01)
